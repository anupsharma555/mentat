...
def format_llama2_prompt(system_message: str, user_message: str) -> str:
    """
    Formats a prompt according to Llama 2 Chat instructions.
    
    For a single message instance with an optional system prompt, the format is:
    ...
    If no system message is provided, the system part is omitted.
    """
    prompt = "<s>[INST] "
    if system_message:
        prompt += "<<SYS>>\n" + system_message.strip() + "\n<</SYS>>\n\n"
    prompt += user_message.strip() + " [/INST]" + "Answer: "
    return prompt
...
def get_candidate_logprobs_for_prompt(
    ...
    """
    Wraps the provided prompt in the Llama 2 Chat format, tokenizes it, 
    and computes the next-token log probabilities for each candidate.
    ...
    # Hotfix: Remove "Answer: " string to add it after ent of instruction to get better model performance.
    prompt = prompt.strip()[:-24]
    full_prompt = format_llama2_prompt(system_message, prompt)
    encoded_input = tokenizer(full_prompt, return_tensors="pt", truncation=True).to(device)
    input_ids = encoded_input["input_ids"]

    with torch.no_grad():
        outputs = model(input_ids)
        # Extract logits for the token position immediately following the prompt.
    ...
    # candidate_logprobs = [319, 350, 315, 36, 382]
...
def evaluate_dataset_on_model(dataset, client, model_name="llama2"):
    """
    Expects each sample in the dataset to have:
      - "prompt_mcq": string with the multiple choice question prompt.
    ...
    for sample in tqdm(dataset):
        ...
        # Convert logprobs -> normalized probs and compute cross-entropy.
...
def main():
    # Select one of the Llama 2 or Llama 2 Chat models:
